...
export function BudgetCalculator() {
  ...
  const calculateEstimate = (e: React.FormEvent) => {
    ...
    const rule = siteConfig.defaultPricing[form.serviceType as keyof typeof siteConfig.defaultPricing] 
                 || siteConfig.defaultPricing.LEVE;
...
Gostaria de confirmar o atendimento.`;
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Calcular Orçamento Estimado</h2>
      
      <form onSubmit={calculateEstimate} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Categoria do Serviço</label>
            <select 
            ...
            >
              ...
              <option value="UTILITARIO">Guincho Utilitário (SUVs, Pickups)</option>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Distância Estimada (km)</label>
            <input 
              type="number" 
              required
              min="1"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Origem (Cidade/Bairro)</label>
            <input 
              type="text" 
              required
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Destino (Cidade/Bairro)</label>
            <input 
              type="text" 
              required
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
      ...
      {estimate !== null && (
        <div className="mt-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <h3 className="text-xl font-bold text-gray-900 mb-4">Resumo da Solicitação</h3>
          <div className="text-3xl font-bold text-orange-600 mb-2">
            R$ {estimate.toFixed(2)}*
          </div>
          <p className="text-xs text-gray-500 mb-6">
            *Este valor é uma estimativa. O valor final poderá variar conforme as condições do atendimento, 
            rota, pedágios, acesso e necessidade de equipamentos extras no local.
          </p>
